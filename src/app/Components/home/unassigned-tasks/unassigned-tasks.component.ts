...
export class UnassignedTasksComponent implements OnInit {
  ...
  tasks: any;
  constructor(private api: GenericService, private fb: FormBuilder, private auth: UserStoreService, private task: TaskService) {
  }
  getTasks() {
  ...
  addAssign() {
    if (this.AssigForm.controls['Assigned'].value == false) { this.AssigForm.controls['id'].setValue(this.slectedUser!.id) }
